...
const Sidebar: React.FC<SidebarProps> = ({ className = '' }) => {
  const { sidebarOpen, setSidebarOpen, sidebarCollapsed, setSidebarCollapsed, favorites, isFavorite, setSelectedCalculator } = useApp();
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-50 transform transition-all duration-300 ease-in-out lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      } ${
        sidebarCollapsed ? 'w-16 lg:w-16' : 'w-80'
      } ${className}`}>
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto">
          <div className={`${sidebarCollapsed ? 'p-2 space-y-2' : 'p-4 space-y-2'}`}>
            ...
            <div className="mb-4">
              {sidebarCollapsed ? (
                <div className="flex flex-col items-center space-y-2">
                  <button
                    ...
                    className="p-3 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 rounded-lg transition-colors w-full flex items-center justify-center"
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">{favorites.length} saved</span>
                    </div>
                  </div>
              ...
              {showFavorites && !sidebarCollapsed && favorites.length > 0 && (
                ...
                return (
                  <div key={category.id} className="group">
                    {sidebarCollapsed ? (
                      <div className="flex flex-col items-center space-y-2">
                        <button
                          ...
                          className={`p-3 rounded-lg transition-all w-full flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 ${
                            isExpanded ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                          }`}
                          title={`${category.name} (${category.count} calculators)`}
                        >
                      ...
                      >
                        <div className="flex items-center space-x-3 flex-1 min-w-0">
                          <div className={`w-10 h-10 rounded-lg ${category.color} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                            <Folder className="h-5 w-5 text-white" />
                          </div>
                    ...
                    {isExpanded && !sidebarCollapsed && (
                      <div className="mt-1 ml-4 pl-3 border-l-2 border-gray-200 dark:border-gray-700 space-y-1 max-h-60 overflow-y-auto">
                        {categoryCalculators.slice(0, 10).map((calculator) => ( // Show first 10
                          <button
                            key={calculator.id}
                            onClick={() => handleCalculatorSelect(calculator)}
                            className="w-full flex items-center justify-between p-2 text-left hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-colors group/calc"
                          >
                            <div className="flex items-center space-x-2 flex-1 min-w-0">
                              <Calculator className="h-4 w-4 text-gray-400 group-hover/calc:text-blue-500 flex-shrink-0" />
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-gray-900 dark:text-white group-hover/calc:text-blue-600 dark:group-hover/calc:text-blue-400 truncate">
                                  {calculator.title}
                                </p>
                            ...
                            <div className="flex items-center space-x-1 flex-shrink-0 ml-2">
                              ...
                              >
                                <Star className="h-3 w-3" fill={isFavorite(calculator.id) ? 'currentColor' : 'none'} />
                              </button>
                              <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                                calculator.complexity === 'Basic' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                calculator.complexity === 'Intermediate' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' :
